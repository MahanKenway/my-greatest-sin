...
def run(corridor_path: Path, build_dir: Path, rate_hz: float) -> None:
    status = preflight()
    ...
    report = {"status": "CUDA CORRIDOR RUN — MODEL RESULT", "corridorSha256": sha256(corridor_path), "rateHz": rate_hz, "inputCount": len(inputs), "nodeCount": len(nodes), "edgeCount": len(sources), "mn9Spikes": int(len(monitor.t)), "boundary": "This is a server-side bounded corridor result, not full FlyWire or FlyBody control."}
...
def main() -> int:
    ...
    run(args.corridor.resolve(), args.build_dir.resolve(), args.rate_hz)
    return 0
