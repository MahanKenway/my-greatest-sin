...
### 9. Multi-food memory, rest and controlled exploration

- **Why isolated:** Multi-target selection, visited-path memory and behavioural state can make a source-topology execution look like unverified cognition if they are folded into neural data. The simulation must expose this as a deterministic presentation/decoder policy.
- **Approach:** Give `Arena` three visible, modelled food targets with fixed values and a shared field-strength control. `WormNavigator` holds a bounded short-term visited-food and recent-path buffer; it temporarily down-weights recently visited food, rests after feeding, then chooses a lower-revisit target. If no target field passes threshold, it produces a seeded, bounded exploration arc biased away from its recent path. `WormBody` low-pass filters turn and stride before applying its visual body wave.
- **Verify:** Pure tests show higher-value target selection, recent-visit avoidance, bounded resting, deterministic exploration and obstacle priority. Browser observation must display target/value/memory/behaviour, show smoother turns and preserve `279 N / 6,261 E` plus staged fly isolation.

### 10. Network-first C. elegans motion correction

- **Decision:** Remove the forage rock, collision correction, target-selection, route-memory, rest and exploration policy. They created independent steering after neural decoding and therefore cannot be presented as movement determined by the source network.
- **Replacement:** Garden food markers remain as explicitly modelled sensory-field inputs. The source topology executes first; only its disclosed motor decoder supplies `MotorFrame` to the body. The decoder has no baseline forward-speed floor and no direct wind-to-turn term. Visual smoothing may reduce render jitter but cannot add turn, speed, target choice or collision response.
- **Open-source boundary:** `openworm/c302` (MIT) is retained as a documented NeuroML/connectivity reference only. `openworm/CE_locomotion` is not imported because its GitHub metadata does not state a license.
