...
### Connected hierarchy repair candidate

The follow-up **NeuroMechFly v5** derivative keeps the same 65 source mesh parts but now preserves their source rest-pose parent hierarchy: every `nmf__<joint>` mesh is parented to its matching `nmf_pivot__<joint>`, and every source pivot follows its inspected parent bone. Local inspection confirms the 65 meshes remain intact with no armature or source action added. Two clean local renders show one continuous fly silhouette with wings, legs and body connected at rest. This is a geometry/hierarchy repair only; it does not turn the source model into an authored behavioural animation.

## Replacement research — FlyBody

The public [TuragaLab/flybody](https://github.com/TuragaLab/flybody) repository is **Apache-2.0** and describes itself as an anatomically detailed *Drosophila melanogaster* body model for MuJoCo physics, developed collaboratively by Google DeepMind and HHMI Janelia. The locally cloned public source includes a 20,751,800-byte `drosophila.blend` plus named OBJ components for head, thorax, abdomen, bilateral wing membranes, and every bilateral leg chain. Blender 4.0.2 inspection ran with `--disable-autoexec` and found 69 meshes / 145,814 polygons, one armature and zero Actions. A controlled local render visibly shows an intact head, thorax, segmented abdomen, two wings, and natural multi-segmented legs in a coherent rest pose. The active clean derivative is `/manus-storage/flybody-articulated-presentation_4068d305.glb`: 68 source mesh parts, 86 rest-pose pivots and SHA-256 `b51e0f7cf9d845fc7315080bf04745991c02e69672c1dc997267b500538d7605`. Browser validation showed the head facing forward with legs below the thorax and two wings attached. Its source provides real anatomical hierarchy but no authored locomotion clip; all browser motion therefore remains explicitly **MODELLED MAPPING**.

## Replacement research — BAAIWorm

The public [Jessie940611/BAAIWorm](https://github.com/Jessie940611/BAAIWorm) repository is **Apache-2.0** and exposes `Metaworm/data/worm_all.obj` without login. It was downloaded only for local inspection; no source code or simulation was executed. The OBJ imports as a small, dark, low-detail mesh with a weakly identifiable head/tail silhouette in a controlled render. It does not meet the user's visual requirement for a natural, detailed and unambiguous *C. elegans* body, so it is **rejected** as a presentation replacement. The active replacement is a detailed handcrafted, single-mesh body at `/manus-storage/celegans-modelled-continuous-presentation-v3_00dd7cb4.glb`, SHA-256 `276b1955b08a361cb9086c9c97209087d9074950b8cfe788e1ab299dcc2ff7d9`, with a tapered tail, rounded head collar, subtle annuli and a four-curve travelling-wave Action. Both the body and the wave are clearly labelled **MODELLED MAPPING**; this choice does not alter the separately verified 279-neuron / 6,261-edge C. elegans topology runtime.
